...
## Disclaimers
This software and its outputs are provided for research and educational purposes only. They must not be used for clinical diagnosis, treatment decisions, or any form of patient management. The results generated by this model are not intended to replace professional medical judgment or guide individual patient care. Users are advised that the algorithm may not meet their specific requirements and that its performance can vary depending on imaging quality and population. Although the model is based on medical research and validated datasets, we cannot guarantee the accuracy, completeness, or currency of its outputs. The user assumes full responsibility for any decisions or actions taken based on these results.
