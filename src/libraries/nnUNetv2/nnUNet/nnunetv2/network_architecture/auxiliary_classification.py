...
class AuxiliaryClassifier(nn.Module):
    ...
    def __init__(self, 
        ...
        self.linear = nn.Linear(input_feature_channels, num_classes if num_classes != 2 else 1)
...
class PlainConvUNetAuxiliaryClassifier(PlainConvUNet):
    def __init__(self,
                 input_channels: int,
                 n_stages: int,
        ...
        self.auxiliary_classifier = AuxiliaryClassifier(self.encoder.output_channels[-1], 
                                                        num_classes_aux_classif, global_pooling, dropout_op, dropout_op_aux_kwargs)
    
    def forward(self, x):
        skips = self.encoder(x)
        # Only return the output of auxiliary_classifier at train train time
        ...
        if not self.decoder.deep_supervision:
            ...
            return self.decoder(skips), self.auxiliary_classifier(skips[-1])
    

class ResidualEncoderUNetAuxiliaryClassifier(ResidualEncoderUNet):
    def __init__(self,
                 input_channels: int,
                 n_stages: int,
                 features_per_stage: Union[int, List[int], Tuple[int, ...]],
