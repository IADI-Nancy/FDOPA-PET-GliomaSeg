...
class nnUNetDataLoader2DAuxiliaryClassifier(nnUNetDataLoaderBase):
    """
    Gather and return the label for auxiliary classification
    """        
    def generate_train_batch(self):
        selected_keys = self.get_indices()
        # preallocate memory for data and seg
            ...
            with torch.no_grad():
                with threadpool_limits(limits=1, user_api=None):
                    ...
                    for b in range(self.batch_size):
                        ...
                        images.append(tmp['image'])
                        segs.append(tmp['segmentation'])
                        # TODO : should we check after transform that there is still some seg and change label accordingly ?
                    data_all = torch.stack(images)
                    if isinstance(segs[0], list):
                        seg_all = [torch.stack([s[i] for s in segs]) for i in range(len(segs[0]))]
                    else:
...
class nnUNetDataLoader2DAuxiliaryRegressor(nnUNetDataLoaderBase):
    """
    Gather and return the label for auxiliary classification
    """        
    def generate_train_batch(self):
...
class nnUNetDataLoader2DSelfSupervision(nnUNetDataLoaderBase):
    """
    Add the return of the image beform transform to allow for self supervision
    """       
    def generate_train_batch(self):
        selected_keys = self.get_indices()
        # preallocate memory for data and seg
