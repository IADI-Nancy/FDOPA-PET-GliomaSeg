...
class nnUNetTrainerAuxiliaryClassifier(nnUNetTrainer):
    def __init__(self, plans: dict, configuration: str, fold: int, dataset_json: dict, unpack_dataset: bool = True,
                 device: torch.device = torch.device('cuda')):
        super().__init__(plans, configuration, fold, dataset_json, unpack_dataset, device)
        self.logger.my_fantastic_logging['auxiliary_accuracy'] = list()
        self.aux_labels = list(self.dataset_json['auxiliary_classifier_labels'].values())
        self.num_aux_classif_classes = len(self.aux_labels)

    def _build_loss(self):
        # Manage Segmentation loss as usual
        seg_loss = super()._build_loss()
        if self.num_aux_classif_classes == 2:
            aux_classif_loss = torch.nn.BCEWithLogitsLoss()
        else:
            label_smoothing = 0
            aux_classif_loss = torch.nn.CrossEntropyLoss(label_smoothing=label_smoothing)
    ...
    def validation_step(self, batch: dict) -> dict:
        ...
        if self.num_aux_classif_classes == 2:
            predicted_aux_class = (torch.sigmoid(output_aux) > 0.5).long()
        else:
            predicted_aux_class = output_aux.argmax(1)
        cm = skm.confusion_matrix(target_aux.detach().cpu().numpy(), predicted_aux_class.cpu().numpy(), labels=self.aux_labels)
        if self.num_aux_classif_classes == 2:
            tn_aux, fp_aux, fn_aux, tp_aux = cm.ravel()
        else:
            tp_aux = np.diag(cm)
            fp_aux = cm.sum(axis=0) - tp_aux
            fn_aux = cm.sum(axis=1) - tp_aux
            tn_aux = cm.sum() - (fp_aux + fn_aux + tp_aux)

        out = {'loss': l.detach().cpu().numpy(), 'tp_hard': tp_hard, 'fp_hard': fp_hard, 'fn_hard': fn_hard,
               'tp_aux': tp_aux, 'fp_aux': fp_aux, 'fn_aux': fn_aux, 'tn_aux': tn_aux}
        out.update({'comp_loss_%s' % l_i: comp_l[l_i].detach().cpu().numpy() for l_i in comp_l})
        return out
    ...
    def on_validation_epoch_end(self, val_outputs: List[dict]):
        ...
        tp_aux = np.sum(outputs_collated['tp_aux'], 0)
        fp_aux = np.sum(outputs_collated['fp_aux'], 0)
        fn_aux = np.sum(outputs_collated['fn_aux'], 0)
        tn_aux = np.sum(outputs_collated['tn_aux'], 0)
        ...
        if self.is_ddp:
            ...
            tps_aux = [None for _ in range(world_size)]
            dist.all_gather_object(tps_aux, tp_aux)
            tp_aux = np.vstack([i[None] for i in tps_aux]).sum(0)

            fps_aux = [None for _ in range(world_size)]
            dist.all_gather_object(fps_aux, fp_aux)
            fp_aux = np.vstack([i[None] for i in fps_aux]).sum(0)

            fns_aux = [None for _ in range(world_size)]
            dist.all_gather_object(fns_aux, fn_aux)
            fn_aux = np.vstack([i[None] for i in fns_aux]).sum(0)

            tns_aux = [None for _ in range(world_size)]
            dist.all_gather_object(tns_aux, tn_aux)
            tn_aux = np.vstack([i[None] for i in tns_aux]).sum(0)
        ...
        if isinstance(tp_aux, np.ndarray):
            aux_accuracy = np.sum(tp_aux) / (np.sum(tp_aux + tn_aux + fp_aux + fn_aux) / tp_aux.size)
        else:
            aux_accuracy = (tp_aux + tn_aux) / (tp_aux + tn_aux + fp_aux + fn_aux)
        self.logger.log('mean_fg_dice', mean_fg_dice, self.current_epoch)
        self.logger.log('dice_per_class_or_region', global_dc_per_class, self.current_epoch)
        self.logger.log('auxiliary_accuracy', aux_accuracy, self.current_epoch)  
    ...
    def on_epoch_end(self):
        ...
        self.print_to_log_file('Auxiliary classifier accuracy', np.round(self.logger.my_fantastic_logging['auxiliary_accuracy'][-1], decimals=4))
        self.print_to_log_file(
    ...
    # 1. In nnUNetTrainerAuxiliaryClassifier : modify perform_actual_validation
    ...
    # 3. modify nnunetv2.evaluation.evaluate_predictions.compute_metrics so that it compute accuracy etc for the auxiliary classifier
    # Something else? 
    
class nnUNetTrainerAuxiliaryClassifier_higher_weight(nnUNetTrainerAuxiliaryClassifier):
    def __init__(self, plans: dict, configuration: str, fold: int, dataset_json: dict, unpack_dataset: bool = True,
                 device: torch.device = torch.device('cuda')):
        super().__init__(plans, configuration, fold, dataset_json, unpack_dataset, device)
    ...
    def _build_loss(self):
        # Manage Segmentation loss as usual
        seg_loss = super(nnUNetTrainerAuxiliaryClassifier, self)._build_loss()
        if self.num_aux_classif_classes == 2:
            aux_classif_loss = torch.nn.BCEWithLogitsLoss()
        else:
            label_smoothing = 0
