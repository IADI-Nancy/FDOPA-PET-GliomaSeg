...
from nnunetv2.training.dataloading.data_loader_2d import nnUNetDataLoader2DSelfSupervision
from nnunetv2.training.dataloading.data_loader_3d import nnUNetDataLoader3DSelfSupervision
from nnunetv2.training.loss.compound_losses import Seg_and_Auxiliary_loss
from nnunetv2.training.loss.contrastive_loss import NTXentLoss
...
class nnUNetTrainerContrastiveSupervision(nnUNetTrainer):
    """
    http://arxiv.org/abs/2207.05306
    """
    ...
    def _build_loss(self):
        ...
        # Contrastive loss + wrap in DeepSupervisionWrapper (for weights see publication if increasing weights with depth or equal weights)
        contrastive_loss = NTXentLoss(temperature=0.1)

        if self._do_i_compile():
            contrastive_loss = torch.compile(contrastive_loss)

        # we give each output the same weight as in the original publication
        # TODO more weights on bottleneck ? 
        if self.enable_deep_supervision:
            deep_supervision_scales = self._get_deep_supervision_scales()
            weights = np.array([1 for i in range(len(deep_supervision_scales) + 1)])

            # now wrap the loss
            contrastive_loss = DeepSupervisionWrapper(contrastive_loss, weights)
        
        total_loss = Seg_and_Auxiliary_loss(seg_loss, contrastive_loss, weights={'seg': 1, 'aux': 0.1})
        return total_loss
    ...
    def get_dataloaders(self):
        ...
        if dim == 2:
            dl_tr = nnUNetDataLoader2DSelfSupervision(dataset_tr, self.batch_size,
                                                      initial_patch_size,
                                                      self.configuration_manager.patch_size,
                                                      self.label_manager,
                                                      oversample_foreground_percent=self.oversample_foreground_percent,
                                                      sampling_probabilities=None, pad_sides=None, transforms=tr_transforms)
            dl_val = nnUNetDataLoader2DSelfSupervision(dataset_val, self.batch_size,
                                                       self.configuration_manager.patch_size,
                                                       self.configuration_manager.patch_size,
                                                       self.label_manager,
                                                       oversample_foreground_percent=self.oversample_foreground_percent,
            ...
            dl_tr = nnUNetDataLoader3DSelfSupervision(dataset_tr, self.batch_size,
                                                      initial_patch_size,
                                                      self.configuration_manager.patch_size,
                                                      self.label_manager,
                                                      oversample_foreground_percent=self.oversample_foreground_percent,
                                                      sampling_probabilities=None, pad_sides=None, transforms=tr_transforms)
            dl_val = nnUNetDataLoader3DSelfSupervision(dataset_val, self.batch_size,
                                                       self.configuration_manager.patch_size,
                                                       self.configuration_manager.patch_size,
                                                       self.label_manager,
                                                       oversample_foreground_percent=self.oversample_foreground_percent,
    ...
    def set_deep_supervision_enabled(self, enabled: bool):
        ...
        mod.encoder.deep_supervision = enabled
    ...
    def validation_step(self, batch: dict) -> dict:
        ...
        tp, fp, fn, _ = get_tp_fp_fn_tn(predicted_segmentation_onehot, target_seg, axes=axes, mask=mask_seg)
...
class nnUNetTrainerContrastiveSupervision_higher_weight(nnUNetTrainerContrastiveSupervision):
    def __init__(self, plans: dict, configuration: str, fold: int, dataset_json: dict, unpack_dataset: bool = True,
                 device: torch.device = torch.device('cuda')):
        super().__init__(plans, configuration, fold, dataset_json, unpack_dataset, device)
    ...
    def _build_loss(self):
        # Manage Segmentation loss as usual
        seg_loss = super(nnUNetTrainerContrastiveSupervision, self)._build_loss()
        ...
        total_loss = Seg_and_Auxiliary_loss(seg_loss, contrastive_loss, weights={'seg': 1, 'aux': 0.5})
        return total_loss