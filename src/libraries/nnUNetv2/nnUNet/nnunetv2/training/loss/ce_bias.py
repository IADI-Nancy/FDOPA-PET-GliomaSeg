# from https://github.com/by-liu/SegLossBias/blob/main/losses_nnunet/ce_bias.py and adapted for nnunetv2
...
class SegLossBiasCompoundLoss(nn.Module):
    """
...
class CrossEntropyWithL1(SegLossBiasCompoundLoss):
    """
...
class FocalWithL1(SegLossBiasCompoundLoss):
    def __init__(self, *args, **kwargs) -> None:
        super().__init__(*args, **kwargs)
        self.focal = FocalLoss(apply_nonlin=softmax_helper_dim1)
...
class CrossEntropyWithKL(SegLossBiasCompoundLoss):
    """
    Cross entropy loss with region size priors measured by l1.
    The loss can be described as:
        l = CE(X, Y) + alpha * |gt_region - prob_region|
