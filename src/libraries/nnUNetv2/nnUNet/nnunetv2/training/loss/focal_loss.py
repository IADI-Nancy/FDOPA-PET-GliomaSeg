...
class FocalTversky_loss(nn.Module):
    ...
    def __init__(self, gamma=0.75, **tversky_kwargs):
        ...
        if gamma > 1:
            ...
            self.gamma = gamma
        # Ensure reduction is 'none' for FocalTverskyLoss
        tversky_kwargs['reduction'] = 'none'
        self.tversky_loss = TverskyLoss(**tversky_kwargs)

    def forward(self, x, y, loss_mask=None):
        tversky_loss = 1 + self.tversky_loss(x, y, loss_mask) # = 1-tversky(x, y, loss_mask)
        focal_tversky = torch.pow(torch.clip(tversky_loss, min=1e-6, max=1), self.gamma)
